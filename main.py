...
class Estimator():
    ...
    def _build_pc_network(self):
        ...
        # 1 - value, 4 - action
        h_pc_deconv_v = tf.contrib.layers.conv2d_transpose(h_pc_fc1_reshaped, 1, 4, 2, activation_fn=tf.nn.relu, padding='VALID')
        h_pc_deconv_a = tf.contrib.layers.conv2d_transpose(h_pc_fc1_reshaped, 4, 4, 2, activation_fn=tf.nn.relu, padding='VALID')
    ...
    def run_n_steps(self, t_max, sess):
        ...
        for _ in range(t_max):
            ...
            frame = ExperienceFrame(
                state=self.state, action=action, reward=reward, next_state=next_state, done=done, pixel_change=pixel_change)
            ...
            if done:
                print("agent {}, episode {}, total_reward {}, episode_length {}, action distr {}".format(
                    self.name, self.episode_counter, int(self.total_reward*100), self.episode_length, np.bincount(self.action_counter)))
                
                summaries = sess.run(self.summaries_op, {self.ep_reward_pl: int(self.total_reward*100), self.ep_length_pl: self.episode_length})
...
agent = Agent(name=name_of_run, env=env, model_net=model, discount_factor=0.99, t_max=T_MAX, summary_writer=summary_writer)
